...
export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(`${AGENT_URL}/v1/classify`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        text: body.text || "",
        url: body.url || "",
        hintTrack: body.hintTrack || "",
        openerMap: body.openerMap || {},
      }),
      signal: ctrl.signal,
    });
    ...
    if (FALLBACK) {
      req.body = body;
      const legacy = await import("../ai/classify.js");
      console.warn("[agent] classify fallback:", e.message);
      return legacy.default(req, res);
    }
    res.status(502).json({ error: e.message || "Agent unavailable" });
  } finally {
    clearTimeout(timer);
  }
}
